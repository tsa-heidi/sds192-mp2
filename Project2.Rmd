...
final_df <- select(final_df,cand_name, sum, ge_winner, incumbent) %>%
  arrange(sum) %>%
  filter(sum>=40000) %>%
  mutate(incumbent01= ifelse(incumbent==TRUE, 1, 0))
...
```{r}
my_plot3 <- ggplot(data= test2, aes(x = reorder(cand_name,-sum), y = sum, fill = ge_winner)) + 
...
my_plot3 + theme(axis.text.x = element_text(angle = 0)) 
options(scipen = 100000)

my_plot4 <- ggplot(data= test3, aes(x = reorder(cand_name,-sum), y = sum, fill = ge_winner)) + 
  geom_bar(position = "dodge",stat = "identity")+
  coord_flip() +
  ggtitle("Does contribution amount influence house election success?")+
  xlab("")+
...
my_plot4 + theme(axis.text.x = element_text(angle = 0)) 
options(scipen = 100000)
```
Creating a data graphic
```{r}
plot1 <- ggplot(data = test1, aes(x=incumbent, y= ge_winner, size=total)) + geom_point()
plot1
```

```{r}
my_plot1 <- ggplot(data= final_df, aes(x = reorder(cand_name,-sum), y = sum, fill = ge_winner)) + 
  geom_bar(position = "dodge",stat = "identity")+
  coord_flip() +
  ggtitle("Does contribution amount influence house election success?")+
  xlab("")+
...
my_plot1 + theme(axis.text.x = element_text(angle = 0)) 
options(scipen = 100000)

my_plot2 <- ggplot(data= final_df, aes(x = reorder(cand_name,-sum), y = sum, fill = ge_winner)) + 
  geom_bar(position = "dodge",stat = "identity")+
  coord_flip() +
  ggtitle("Does contribution amount influence house election success?")+
  xlab("")+
  labs(fill="")+
  scale_fill_discrete(labels=c("Lost","Won"))+
  scale_y_continuous(name="Money Spent (millions of USD)",breaks=c(0,2000000,4000000),labels=c("0$", "2,000,000$", "4,000,000$")) + facet_wrap(~incumbent)
...
```{r}
NY <- function1_candidates(2012, "NY")
```
...
NY Republicans
```{r}
NYR <- function1_candidates(2012, "MA")
...
```{r}
my_plotNYR <- ggplot(data= NYRfinal, aes(x = cand_name, y = sum, fill = ge_winner)) + 
  geom_bar(position = "dodge",stat = "identity")+
  coord_flip() +
