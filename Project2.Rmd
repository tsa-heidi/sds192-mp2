...
final_df <- inner_join(contr_cand_joined,
  ...
  select(cand_name, 
         ...
         incumbent) %>%
  arrange(sum) %>%
  filter(sum >= 50000) %>% # we chose to filter out candidates who received less than 50 000 dollars 
  filter(!is.na(sum))
```
...
my_plot1 <- ggplot(data = final_df, 
  ...
  scale_y_continuous(name = "Money Spent (millions of USD)",                breaks=c(0,2000000,4000000),
                     labels=c("0$", 
                              "2,000,000$",
                              "4,000,000$")) + theme(axis.text.x = element_text(angle = 0))
```
...
function8_facet <- function(function7plotname) {
  function7plotname + facet_wrap(~incumbent)
}
```
