...
The House of Representatives is composed of politicians who represent their congressional districts in their respective states. The number of congressional districts allocated to each of the 50 states is based on the state’s population size. Each district is entitled to one representative, and their are multiple districts in each state. Thus, there will be multiple candidates running for house election within each party for a particular state. Our analysis focuses on these candidates running for election to The House of Representatives. 

*Method of investigation*

Our analysis took part in 4 stages:

1) We focused on candidates within a particular state, year and party. We included party as variable to control for the fact that the probability of winning may be influenced by whether a state is already 'blue' or 'red' from the last election cycle. 
We selected New York Democrats in 2012 and wrangled this from the FEC “Candidates” data set. 

2) We created a list of the candidates’ IDs in our wrangled candidates data set, and used these IDs to filter out their corresponding contributions from the FEC “contributions” data set. The two datasets, “Candidates” and “Contributions”, were joined by their matching candidate IDs. 

3) In the House Elections dataset, we filtered for our variables of interest: candidate ID (under the name fec_id), whether the candidate won or lost, and whether they were an incumbent (this choice will become clear in our analysis). The House Election dataset was then filtered according to the candidate IDs we wrangled earlier, which in our particular case this was the candidate IDs of New York Democrats in 2012.

4) Our final data frame was created by joining the data sets; Contributions, Candidates and House Elections by our selected candidate IDs. The total amount of contributions to each candidate was calculated. Candidates with no data were omitted.
...
Considering that our sample size becomes too small to conclude any trend once we wrangled for year, party and state, we created functions to generalize our analysis. Since a greater number of candidates run in states with large populations, we chose the three biggest states by population in each region: New York (East), California (West) and Texas (South). 

*Results*

In our initial analysis, the graphs that do not take into account whether a candidate is an incumbent or not, we noticed that there was no consistent trend between contribution amount and wins. For example, in NY democrats (NYD), Hochul received the greatest amount in donations yet did not win. Similarly, for NY republicans (NYR), Buerkle and Hayworth got the greatest amount in contribution yet lost. We see this same result in California democrats and republicans (CAD/R) and Texas republicans. There are also candidates who received relatively little in donations (less than 2 million) and still won. Thus, there seems to be no obvious direct correlation between amount of donations received and probability of winning.

Therefore, we decided to further our analysis by faceting our initial graphs by whether the candidate was an incumbent or not. It appears that if you are an incumbent, you are more likely to get larger donations (see NYR/Dplot2, CARplot2, TXRplot2), but not necessarily win. Similarly, if you are an incumbent, it seems you are more likely to win even if you have a smaller amount in contributions (see NYD/Rplot2, TXRplot2, CADplot2). This result could be explained by the fact that, firstly, incumbents are more established and therefore donations matter less. Secondly, we recognize that most of the candidates who run are probably incumbents which could be skewing our results. If you are not an incumbent, it seems that the more donations you have, the more likely you are to win as the blue bars are longer for non-incumbents who won (see NYDplot2, CADplot2, TXDplot2)
